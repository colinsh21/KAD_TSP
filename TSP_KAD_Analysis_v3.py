...
class TSP(object):
    def __init__(self, start=0, dissipation=.2, tolerance=.2, alpha=1.0, beta=1.0, explore=1.0, n_cities=4, force=None):
        # inputs
...
def run_ACO(steps=100, to_step_lim=False, cities=5, tolerance=0.7, alpha=1.0, beta=1.0, dissipation=0.2, explore=1.0, force=None):
    t = TSP(start=0, tolerance=tolerance, alpha=alpha, beta=beta, dissipation=dissipation, explore=explore,
            n_cities=cities, force=force)
...
for i in xrange(iters):
    tol=1.0
    tsp, routes = run_ACO(steps=steps, to_step_lim=True, cities=num_cities, explore=1.0, tolerance=tol)
    tsp_list.append(tsp)
...
for i in xrange(iters):
    ...
    t_d_contex.append(tsp.d_change[0])
...
ana_label={'out':'Out Degree', 'in':'In Degree', 'between':'Betweenness',
           'hits-a':'HITS-Authority', 'hits-h':'HITS-Hub'}
...
plt.ylabel('In-Degree Centrality Sum')
plt.xlabel('Time')
...
plot_df.plot.bar(x=cent_mean['s'].unique(), y=['Learned Context'], ax=a, alpha=alpha,
                 color='b', figsize=figsize, width=w, edgecolor=ec)
plt.ylabel('HITS-Authority Centrality Sum')
plt.xlabel('Time')
plt.title('Action Layer: Context Learning')
# plt.legend(seg_labels, loc='best')
plt.xticks(np.arange(0, steps + 1, float(tick_interval)), range(0, steps + 1, tick_interval))
...
plt.ylabel('HITS-Authority Centrality Mean (with 95% CI)')
plt.xlabel('Time')
plt.title('Action Layer: Context Learning')
...
plt.ylabel('Out-Degree Centrality Mean (with 95% CI)')
plt.xlabel('Time')
...
#HITS-Hub K Mean TSP Sol, Context Non-Sol
col1='TSP Edges Solution Mean'
col2='TSP Edges Non-Solution Mean'
col_names=['Solution', 'Non-Solution']
ana='hits-h'
...
plt.legend(['Decision-making','Scoring','Prototyping','Context Learning', 'TSP Paths Learning'], loc='best')
plt.xticks(np.arange(0, steps + 1, float(tick_interval)), range(0, steps + 1, tick_interval))
if save:
...
if dec_line_on:
    plt.axvline(x=dec_time,color=dc, linewidth=2, label='Context Decision')
...
plt.ylabel('Out-Degree Centrality Sum')
plt.xlabel('Time')
...
plt.legend(['HITS-Hub','Out Degree','Context Decision'],loc='best')
if save:
