...
def KAD_analysis(t,interval=1):
    ...
    for s, kad in t.history_k.iteritems():
        ...
        if s != 0:
            ...
            results['hits-h'][s] = h
            results['hits-a'][s] = a
            results['out'][s] = nx.out_degree_centrality(kad) #kad.out_degree()
            results['in'][s] = nx.in_degree_centrality(kad) # kad.in_degree()
...
for i in xrange(iters):
    tsp, routes = run_ACO(steps=steps, to_step_lim=True, cities=num_cities, explore=1.0, tolerance=.9)
    tsp_list.append(tsp)
...
for ana in cent_o['centrality'].unique():
    ...
    plot_0.plot(x='s', y=['Strongest Path Error'], title='Analysis-{}: Path Error at 0 Offset'.format(ana))
    plt.fill_between(plot_0['s'].values,plot_0['Min Error'],plot_0['Max Error'],facecolor='blue',alpha=.5, label='Confidence Interval')
    ...
    for l in layers:
        ...
        plot_df.plot.bar(x='s',
                         ...
                         stacked=True,width=1.0, color=color_list,
