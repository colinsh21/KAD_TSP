...
def influence_chart(t,results):
    ...
    for ana, results_series in results.iteritems():
        ...
        c = [cmap(float(i) / len(t.d)) for i in xrange(len(t.d))]
...
def ML_path_comparison_KAD(tsp_object, results_series, title,comparison_base=0):
    ...
    for s, tsp in tsp_series.iteritems():  # go through time steps
        ...
        for u,v,d in tsp.edges(data=True):
            ...
            e_ph = np.power(tsp[u][v]['p'], tsp_object.alpha)* np.power(e_h,tsp_object.beta)
            state_edges['ph'][(u,v)]=e_h
        ...
        for route in itertools.permutations(nodes):  # calc mle for each route
            ...
            for e in e_r:  # selected step
                ...
                # e_ph = np.power(tsp[e[0]][e[1]]['p'], tsp_object.alpha) * np.power(e_h,
                #                                                                    tsp_object.beta)  # tsp[e[0]][e[1]]['p']
                # e_ph=tsp[e[0]][e[1]]['p']
                ...
                for tail in tsp.nodes():
                    ...
                    #
                    # tot_ph += np.power(tsp[e[0]][tail]['p'], tsp_object.alpha) * np.power(e_h,
                    #                                                                       tsp_object.beta)  # tsp[e[0]][tail]['p']
                    ...
                    #
                    # for last_node in tsp_object.last['dist'][(e[0], tail)][::-1]:  # iterate to get centrality results
                    #     if last_node in results_series[s]:
                    #         tot_cent += results_series[s][last_node]
                    #         break
