...
for i in xrange(iters):
    tol_nd=1.0
    tsp, routes = run_ACO(steps=steps, to_step_lim=True, cities=num_cities, explore=1.0, tolerance=tol_nd,dissipation=0.2)
    tsp_list.append(tsp)
    for r,s in zip(routes,xrange(len(routes))):
        scores.append([tol_nd,s,r[1]])

print 'decision'
t_d_contex=[] #Track when the first path is locked in!!
tol_d=.9
for i in xrange(iters):
    tsp, routes = run_ACO(steps=steps, to_step_lim=True, cities=num_cities, explore=1.0, tolerance=tol_d,dissipation=0.2)
    tsp_list.append(tsp)
    for r,s in zip(routes,xrange(len(routes))):
        scores.append([tol_d,s,r[1]])
