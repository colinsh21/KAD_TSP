...
for i in xrange(iters):
    tsp, routes = run_ACO(steps=steps, to_step_lim=True, cities=num_cities, explore=1.0, tolerance=0.7)
    tsp_list.append(tsp)
...
seg_labels=['Decision','Score','Transition Steps','Start Edges', 'TSP Edges']
...
path_0_offset = path_mean.loc[path_mean['offset'] == 0]
...
color_list = cmap(np.linspace(edge_range_bottom, edge_range_top, len(cent_o['centrality'].unique())))

for ana,color in zip(cent_o['centrality'].unique(),color_list):
...
plt.legend(['HITS-Authority','In Degree','HITS-Hub','Out Degree'],loc='best')
if save:
