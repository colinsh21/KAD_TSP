...
class TSP_2diss(object):
    def __init__(self, start=0, diss_old=.2, diss_new=.2, tolerance=.2, alpha=1.0, beta=1.0, explore=1.0, n_cities=4, force=None, damage_step=30, rug=1.0):
        # inputs
        ...
        self.d_edges = []  # list of selected edges
...
class TSP_1diss(object):
    def __init__(self, start=0, dissipation=.2, tolerance=.2, alpha=1.0, beta=1.0, explore=1.0, n_cities=4,
                 force=None, damage_step=30, rug=1.0, run_ID=0, label='SE',criteria='diff'):
        # Method
        self.method='iter'
        ...
        for i in xrange(g.number_of_nodes() - 1):
            ...
            for n in g.nodes():
                if n not in tabu:
                    ...
                    if (g[tabu[-1]][n]['p'] == 0.0) & (self.tolerance<=1.0):  # Part of a decision process
                        dec_point = True
